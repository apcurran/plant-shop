...
describe("log in flow", () => {
    ...
    it("should log in user as Bob Doe", () => {
        cy.env(["testUserEmail", "testUserPassword"]).then(
            ({ testUserEmail, testUserPassword }) => {
                ...
                cy.contains("header", /hello, bob/i).should("be.visible");
            },
        );
    });
    ...
    it("should give error message if user email does not exist in db", () => {
        ...
        cy.contains("p", /Email is not found/i).should("be.visible");
    });
    ...
    it("should give error message if user provides a password that is incorrect", () => {
        cy.env(["testUserEmail"]).then(({ testUserEmail }) => {
            ...
            cy.contains(
                "p",
                /Invalid credentials provided. Check your email or password again./i,
            ).should("be.visible");
        });
    });
});
