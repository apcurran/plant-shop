...
function CartTable({ items }) {
    ...
    // Global store state
    const incrementOneItem = useCartStore(
        useCallback((state) => state.incrementOneItem, []),
    );
    const decrementOneItem = useCartStore(
        useCallback((state) => state.decrementOneItem, []),
    );
    const removeItemFromCart = useCartStore(
        useCallback((state) => state.removeItemFromCart, []),
    );

    const itemsSubTotal = items.reduce(
        (total, currVal) => total + currVal.itemTotalPrice,
        0,
    );
    ...
    const totalInfo = (
        <section className="total-info">
            <div className="total-info__group">
                ...
                <span className="total-info__group__span-val">
                    ${itemsSubTotal}
                </span>
                <span className="total-info__group__span">Shipping:</span>
                <span className="total-info__group__span-val">Free</span>
                <span className="total-info__group__span total-info__group__span--total">
                    Total:
                </span>
                <span className="total-info__group__span-val total-info__group__span-val--total">
                    ${itemsSubTotal}
                </span>
                <CheckoutLink />
            </div>
    ...
    const cartTableClasses = isScrolled
        ? "cart-table cart-table--disable-fade"
        : "cart-table";
            ...
            <table onScroll={disableFade} className={cartTableClasses}>
                <thead className="cart-table__thead">
                    <tr className="cart-table__thead__tr">
                        <th className="cart-table__thead__tr__th cart-table__thead__product">
                            Product
                        </th>
                ...
                <tbody className="cart-table__tbody">{itemsArr}</tbody>
            </table>
            {totalInfo}
        </div>
    );
